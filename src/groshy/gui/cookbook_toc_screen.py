...
class RecipeLabel(Label):
    def on_touch_up(self, touch):
        if self.collide_point(*touch.pos):
            App.get_running_app().screen_manager.add_screen(screen_type='recipe', name='recipe1', change=True, new=False, dt=1)
            return True
...
class CookbookToCScreen(Screen):
    ...
    def on_enter(self, *args):
        self.ids.cookbook_title.text = self.cookbook_data.get_display_name(self.cookbook_data.name)
